...
palette), so a palette switch re-tints this popup the next time it opens — no
hardcoded hex here. GTK renders via cairo (software), so unlike the GL-only
tools this works inside the VM.
"""
import os
import sys
import subprocess
...
CSS = f"""
window {{ background-color: {BG}; border: 1px solid {ACCENT}; }}
label {{ color: {ACCENT}; }}
scale {{ padding: 0 2px; }}
scale trough {{
...
scale highlight {{ background-color: {ACCENT}; border-radius: 3px; }}
scale slider {{
